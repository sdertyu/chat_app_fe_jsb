        ...
        <div ref="contactContainer">
            <ScrollBox class="max-h-58 overflow-y-scroll">
                ...
                " v-for="(contact, index) in props.conversation.contacts" :contact="contact" :key="index">
                    ...
                    <template v-slot:menu v-if="store.user &&
                        (props.conversation.admins as number[]).includes(store.user.id) &&
                        contact.id !== store.user.id
                        ">
                        <div>
                            <!--dropdown menu button-->
                            <IconButton title="menu" @click="(event) => handleToggleDropdown(event, index)"
                            ...
                            <Dropdown :close-dropdown="closeDropdowns" :handle-click-outside="handleClickOutside"
                                ...
                                <button class="dropdown-link dropdown-link-primary" aria-label="give admin permissions"
                                    role="menuitem">
                                    Promote to admin
                                </button>
                                <button class="dropdown-link dropdown-link-primary"
                                    aria-label="remove admin permissions" role="menuitem">
                                    Demote to member
                                </button>
                                <button class="dropdown-link dropdown-link-danger" aria-label="remove contacts"
                                    role="menuitem">
                                    Remove contact
                                </button>
                            </Dropdown>
