...
import type { IAttachment, IConversation } from "@/types";

import { computed, inject, onMounted, onUnmounted, ref, watch } from "vue";

import { hasAttachments } from "@/utils";
import { useFocusTrap } from "@vueuse/integrations/useFocusTrap";
import VideoPlayer from "@/components/ui/data-display/VideoPlayer.vue";
import IconButton from "@/components/ui/inputs/IconButton.vue";
import Toolbar from "@/components/ui/data-display/Carousel/Toolbar.vue";
        ...
        <ScaleTransition>
            <div v-show="props.open" class="fixed inset-0 z-10">
                <div v-if="props.startingId" ref="carousel" class="h-full flex flex-col">
                    <!--toolbar-->
                    <Toolbar class="absolute right-0 z-30 mr-5 mt-5"
                    ...
                    <div class="relative w-full h-full flex items-center justify-center overflow-hidden">
                        ...
                        <!--Image-->
                        <img class="absolute w-auto md:max-w-175 xs:max-w-85 cursor-grab transition-[transform,opacity] duration-200"
                            ...
                            :alt="selectedAttachment.name" ref="image" @load="handleImageLoad"
                            @mousedown="handleStartMovingImage" />

                        <!--Video-->
                        <VideoPlayer class="transition-[transform,opacity] duration-200"
                            :class="{ 'opacity-0': imageInvisibility }" :id="'video-player-' + selectedAttachment.id"
                            v-if="selectedAttachment.type === 'video'" :url="selectedAttachment.url"
                            :name="selectedAttachment.name" :thumbnail="<string>selectedAttachment.thumbnail"
                            :key="selectedAttachment.id" @videoLoad="handleImageLoad" />
