...
<template>
    <div class="mb-7 h-7 xs:hidden md:block">
        <a href="#mainContent" title="Skip to main content" @blur="SkipLinkFocused = false"
            @focus="SkipLinkFocused = true" class="outline-none" :class="{
        ...
        <button aria-label="avian logo" :class="{ hidden: SkipLinkFocused }" class="outline-none">
            <img v-if="!store.settings.darkMode" src="@/assets//vectors/logo.svg" class="w-8 h-7"
                alt="gray bird logo" />
            <img v-else src="@/assets/vectors/logo-white.svg" class="w-8 h-7 opacity-40" alt="white bird logo" />
        </button>
    </div>
</template>
