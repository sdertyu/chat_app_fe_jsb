...
<template>
    <LabeledTextInput placeholder="Search.." class="pl-7" :value="props.value"
        ...
        <template v-slot:endAdornment>
            <IconButton v-if="props.value" @click="
                () => {
                    if (props.value) $emit('valueChanged', '');
                }
            " title="clear text" aria-label="clear text" class="ic-btn-ghost-gray m-[.5rem] p-2">
