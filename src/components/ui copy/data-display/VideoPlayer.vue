...
<template>
    <VideoPlayer class="video-player" :src="props.url" :poster="props.thumbnail" @timeupdate="handleTimeChange"
        @volumechange="handleVolumeChange" @loadstart="(event) => $emit('videoLoad', event)">
        <template v-slot="{ player, state }">
            <div class="overlay-container">
                ...
                <div class="pause-start-container">
                    <!--play button-->
                    <button v-if="!state.playing" @click="() => handleToggleVideo(state, player)"
                        class="control-button play-button">
                        <PlayIcon class="icon large" />
                    </button>
                    <!--pause button-->
                    <button v-if="state.playing" @click="() => handleToggleVideo(state, player)"
                        class="control-button pause-button" :class="{ 'opacity-0': state.playing }">
                ...
                <div v-if="started" class="controls-container">
                    <!--audio controls-->
                    <div class="audio-controls-container" @mouseenter="volumeSliderInvisible = false"
                        ...
                        <!--mute button-->
                        <button v-else class="mute-unmute-buttons" @click="() => handleToggleMute(player)">
                            <SpeakerWaveIcon class="icon small" />
                        </button>

                        <!--audio slider-->
                        <div class="volume-slider-container" :style="{ opacity: volumeSliderInvisible ? 0 : 1 }">
                            <RangeSlider @value-changed="
                                ($event) => handleVolumeSliderChange($event, player)
                            " :percentage="volume" class="volume-slider" aria-label="volume change slider" />
                        </div>
                    </div>

                    <!--pause button-->
                    <button v-if="state.playing" class="pause-play-buttons"
                    ...
                    <!--track slider-->
                    <RangeSlider class="track-slider" :percentage="percentage" aria-label="time track slider"
                        @value-changed="($event) => handleTrackInput($event, player, state)" />

                    <!--maximize button-->
                    <button v-if="!fullScreen" @click="
                        () => {
...
@media (max-width: 60.4rem) {
    .video-player {
        ...
        .overlay-container .controls-container .audio-controls-container .volume-slider-container {
            bottom: 60px;
            right: -30%;
            width: 70px;
        }
